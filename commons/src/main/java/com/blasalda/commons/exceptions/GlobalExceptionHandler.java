package com.blasalda.commons.exceptions;

import com.blasalda.commons.dto.CustomErrorResponse;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<com.blasalda.commons.dto.CustomErrorResponse> handleConstraintViolationException(ConstraintViolationException e) {
    ...
    public ResponseEntity<CustomErrorResponse> handleUniqueConstraintViolation(
            DataIntegrityViolationException ex, WebRequest request) {

        String errorMessage = "A record with this unique value already exists.";

        // Optional: Extract specific constraint info from the root cause message
        if (ex.getRootCause() != null && ex.getRootCause().getMessage().contains("duplicate key")) {
            errorMessage = "Database error: Duplicate entry detected.";
        }

        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(new CustomErrorResponse(HttpStatus.CONFLICT.value(),
                        "Ingreso de un dato único duplicado."));
    }*/
